...
- **替换**: 嵌入式 shelf 服务器 → 独立 Server App（电脑 + PostgreSQL）
- **废弃**: UDP 广播发现（扫码 + PIN 替代）
- **废弃**: 客户端/服务器模式切换（手机纯客户端，电脑跑 Server）
...
| 2 | 服务器运行环境 | 电脑上跑，Dart shelf + PostgreSQL |
| 3 | 客户端架构 | 手机纯客户端（SQLite + sync_queue），无服务器模式 |

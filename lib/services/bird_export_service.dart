...
class BirdExportService {
  ...
  Future<File?> exportBirds(List<int> birdIds, AppDatabase db) async {
    ...
    try {
      ...
      debugPrint('[ExportSvc] ensureInitialized 完成 (${sw.elapsedMilliseconds}ms)');

      // ── 1. 并行收集数据 ──
      debugPrint('[ExportSvc] 并行收集鸟详情...');
      final birdsWithDetails = (await Future.wait(
        birdIds.map((id) => db.getWithDetails(id)),
      )).whereType<BirdWithDetails>().toList();
      if (birdsWithDetails.isEmpty) {
        debugPrint('[ExportSvc] 没有找到鸟，返回 null');
        return null;
      }
      debugPrint('[ExportSvc] 鸟详情收集完成: ${birdsWithDetails.length} 只 (${sw.elapsedMilliseconds}ms)');
      ...
      final results = await Future.wait([
        ...
        (db.select(db.activityLogs) ..where((t) => t.birdId.isIn(birdIds))).get(),
        (db.select(db.alertRecords) ..where((t) => t.birdId.isIn(birdIds))).get(),
      ...
      debugPrint('[ExportSvc] 关联数据查询完成: weights=${allWeights.length}, meds=${allMedications.length}, tasks=${allTasks.length}, logs=${allActivityLogs.length}, alerts=${allAlertRecords.length}, photos=${allPhotos.length}, avatars=${allAvatars.length} (${sw.elapsedMilliseconds}ms)');

      debugPrint('[ExportSvc] 查询 breedingPairs...');
      final allPairs = await (db.select(db.breedingPairs)).get();
      final exportPairs = allPairs.where((p) => birdIdSet.contains(p.maleBirdId) && birdIdSet.contains(p.femaleBirdId)).toList();
      debugPrint('[ExportSvc] breedingPairs: ${allPairs.length} total, ${exportPairs.length} exported');

      final pairIds = exportPairs.map((p) => p.id).toSet();
      final allBreedingRecords = pairIds.isNotEmpty
          ? await (db.select(db.breedingRecords)..where((t) => t.pairId.isIn(pairIds))).get()
          : <BreedingRecord>[];
      final recordIds = allBreedingRecords.map((r) => r.id).toSet();
      final allEggs = recordIds.isNotEmpty
          ? await (db.select(db.eggs)..where((t) => t.breedingRecordId.isIn(recordIds))).get()
          : <Egg>[];
      final allMatingEvents = recordIds.isNotEmpty
          ? await (db.select(db.matingEvents)..where((t) => t.breedingRecordId.isIn(recordIds))).get()
          : <MatingEvent>[];
      debugPrint('[ExportSvc] breedingRecords: ${allBreedingRecords.length}, eggs: ${allEggs.length}, matingEvents: ${allMatingEvents.length}');

      final birdIdToUuid = {for (final b in birdsWithDetails) b.bird.id: b.bird.uuid};
      ...
      final dataJson = {
        ...
        'birds': birdsWithDetails.map((BirdWithDetails bwd) {
          ...
          return {
            ...
            'medications': (medsByBird[bid] ?? []).map(_medicationToJson).toList(),
            'tasks': (tasksByBird[bid] ?? []).map(_taskToJson).toList(),
            'activityLogs': (logsByBird[bid] ?? []).map(_activityLogToJson).toList(),
            'alertRecords': (alertsByBird[bid] ?? []).map(_alertRecordToJson).toList(),
            'galleryPhotos': (photosByBird[bid] ?? []).map(_birdPhotoToJson).toList(),
            'avatar': (avatarsByBird[bid] ?? []).map(_birdAvatarToJson).firstOrNull,
          };
        }).toList(),
        'breedingPairs': exportPairs.map((bp) => _breedingPairToJson(bp, birdIdToUuid)).toList(),
        'breedingRecords': allBreedingRecords.map((br) => _breedingRecordToJson(br, pairIdToUuid)).toList(),
        'eggs': allEggs.map((e) => _eggToJson(e, recordIdToUuid, birdIdToUuid)).toList(),
        'matingEvents': allMatingEvents.map((me) => _matingEventToJson(me, recordIdToUuid)).toList(),
      };
      ...
      debugPrint('[ExportSvc] jsonEncode 完成: ${dataJsonStr.length} 字符 (${sw.elapsedMilliseconds}ms)');

      debugPrint('[ExportSvc] 启动 Isolate.run()...');
      final exportPath = await _runPackZip(
        dataJsonStr, photoSrcPaths, photoBirdUuids, storageBaseDir,
      );
      debugPrint('[ExportSvc] Isolate.run() 返回: ${exportPath ?? "null"} (${sw.elapsedMilliseconds}ms)');

      if (exportPath == null) {
        debugPrint('[ExportSvc] 导出失败，isolate 返回 null');
        return null;
      }
      debugPrint('[ExportSvc] ===== 导出完成: $exportPath (${sw.elapsedMilliseconds}ms) =====');
  ...
  // ── O(n+m) 分组辅助 ──
  static Map<int, List<T>> _groupById<T>(List<T> items, int? Function(T) getId) {
  ...
  Map<String, dynamic> _birdToJson(Bird b) => {
        'uuid': b.uuid, 'name': b.name, 'ringNumber': b.ringNumber,
        'birthDate': b.birthDate.toIso8601String(), 'gender': b.gender,
        'sortOrder': b.sortOrder, 'weighIntervalDays': b.weighIntervalDays,
        'manualBaselineG': b.manualBaselineG, 'weaningOverride': b.weaningOverride,
  ...
  Map<String, dynamic> _speciesToJson(Specy s) => {
        ...
        'nestlingEndDays': s.nestlingEndDays, 'juvenileEndDays': s.juvenileEndDays,
  ...
  Map<String, dynamic> _weightToJson(Weight w) => {
        'uuid': w.uuid, 'weightG': w.weightG,
        'recordedAt': w.recordedAt.toIso8601String(), 'recordedBy': w.recordedBy,
        'isFasting': w.isFasting, 'notes': w.notes,
        'createdAt': w.createdAt.toIso8601String(), 'updatedAt': w.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _medicationToJson(Medication m) => {
        'uuid': m.uuid, 'drugName': m.drugName, 'drugType': m.drugType,
        'dosage': m.dosage, 'timesPerDay': m.timesPerDay,
        'startDate': m.startDate.toIso8601String(),
        'endDate': m.endDate?.toIso8601String(), 'notes': m.notes, 'active': m.active,
        'createdAt': m.createdAt.toIso8601String(), 'updatedAt': m.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _taskToJson(Task t) => {
        'uuid': t.uuid, 'roomId': t.roomId, 'assignedUserId': t.assignedUserId,
        'taskType': t.taskType, 'dueDate': t.dueDate.toIso8601String(),
        'deadline': t.deadline?.toIso8601String(), 'status': t.status,
        'completedAt': t.completedAt?.toIso8601String(), 'completedBy': t.completedBy,
        'metadata': t.metadata,
  ...
  Map<String, dynamic> _activityLogToJson(ActivityLog a) => {
        'uuid': a.uuid, 'pluginId': a.pluginId, 'actionType': a.actionType,
        'summary': a.summary, 'details': a.details, 'relatedTaskId': a.relatedTaskId,
        'operatedBy': a.operatedBy, 'operatedAt': a.operatedAt.toIso8601String(),
        'createdAt': a.createdAt.toIso8601String(),
      };

  Map<String, dynamic> _alertRecordToJson(AlertRecord a) => {
        'uuid': a.uuid, 'alertType': a.alertType, 'description': a.description,
        'severity': a.severity, 'isRead': a.isRead, 'isResolved': a.isResolved,
        'resolvedAt': a.resolvedAt?.toIso8601String(),
        'createdAt': a.createdAt.toIso8601String(), 'updatedAt': a.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _birdPhotoToJson(BirdPhoto ph) => {
        'fileName': p.basename(ph.filePath), 'relativePath': ph.filePath,
        'sortOrder': ph.sortOrder, 'mediaType': ph.mediaType,
        'videoFileName': ph.videoFilePath != null ? p.basename(ph.videoFilePath!) : null,
        'createdAt': ph.createdAt.toIso8601String(),
      };

  Map<String, dynamic> _birdAvatarToJson(BirdAvatar a) => {
        'fileName': p.basename(a.filePath), 'relativePath': a.filePath,
        'updatedAt': a.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _breedingPairToJson(BreedingPair bp, Map<int, String> birdIdToUuid) => {
        'uuid': bp.uuid, 'maleBirdUuid': birdIdToUuid[bp.maleBirdId],
        'femaleBirdUuid': birdIdToUuid[bp.femaleBirdId],
        'pairName': bp.pairName, 'status': bp.status,
        'pairedDate': bp.pairedDate.toIso8601String(),
        'separatedDate': bp.separatedDate?.toIso8601String(), 'notes': bp.notes,
        'createdAt': bp.createdAt.toIso8601String(), 'updatedAt': bp.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _breedingRecordToJson(BreedingRecord br, Map<int, String> pairIdToUuid) => {
        'uuid': br.uuid, 'pairUuid': pairIdToUuid[br.pairId],
        'stage': br.stage, 'startDate': br.startDate.toIso8601String(),
        'endDate': br.endDate?.toIso8601String(), 'endReason': br.endReason,
        'notes': br.notes,
        'createdAt': br.createdAt.toIso8601String(), 'updatedAt': br.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _eggToJson(Egg e, Map<int, String> recordIdToUuid, Map<int, String> birdIdToUuid) => {
        'uuid': e.uuid, 'breedingRecordUuid': recordIdToUuid[e.breedingRecordId],
        'laidDate': e.laidDate.toIso8601String(),
        'hatchDate': e.hatchDate?.toIso8601String(), 'status': e.status,
        'chickBirdUuid': e.chickBirdId != null ? birdIdToUuid[e.chickBirdId] : null,
        'notes': e.notes,
        'createdAt': e.createdAt.toIso8601String(), 'updatedAt': e.updatedAt.toIso8601String(),
      };

  Map<String, dynamic> _matingEventToJson(MatingEvent me, Map<int, String> recordIdToUuid) => {
        'uuid': me.uuid, 'breedingRecordUuid': recordIdToUuid[me.breedingRecordId],
        'observedDate': me.observedDate.toIso8601String(), 'notes': me.notes,
...
Future<String?> _packZip(
  ...
  try {
    ...
    try {
      ...
      final exportPath = p.join(tempDir.parent.path, 'weightnest_birds_$ts.wnbirds');
      ...
      encoder.addArchiveFile(ArchiveFile('data.json', dataJsonBytes.length, dataJsonBytes));
      ...
      for (int i = 0; i < photoSrcPaths.length; i += batchSize) {
        ...
        for (int j = i; j < end; j++) {
          batchFutures.add(_processOneFile(
            photoSrcPaths[j], photoBirdUuids[j], storageBaseDir,
          ));
        }
      ...
      encoder.addArchiveFile(ArchiveFile('manifest.json', manifestJsonBytes.length, manifestJsonBytes));
      // Finalise ZIP on disk (writes central directory — tiny, O(file count) not O(file size)).
      encoder.close();
      print('[Isolate] manifest 完成: ${manifest.length} 个文件 (${sw.elapsedMilliseconds}ms)');
      ...
      await sink.addStream(File(tempZipPath).openRead()); // stream, never fully in RAM
      await sink.flush();
      await sink.close();
